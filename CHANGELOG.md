...
- **Item enrichment mirror**: Added a versioned `item_enrichments` current-state table for item summaries, OCR, and X media transcripts, with migration backfill, dual-writes from existing compatibility columns, point item reads that prefer the mirror with compatibility fallback, and FTS indexing/search snippets that hydrate from the mirror.
