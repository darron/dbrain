...
- **Web/MCP/store/source/retrieval structure**: Split the web server into focused route files, separated MCP transport/protocol/tools/schemas/helpers, moved store open/schema/search/tag-search/search-count/predicate/item-read/item-scan/item-write/item-enrichment/item-FTS/source-enrichment/X-hydration/X-media-transcription/media/media-archive/stats/stats-pipeline and source schema/link/enrichment/lookup/evidence/relation/tag/search/repair code into focused files, split X hydration orchestration from fetch policy, quote-tree persistence, client/cookie handling, TweetResult request metadata, and snapshot parsing, extracted source-enrichment options, workers, persistence, summary execution/prompt/policy, failure persistence/classification/preflight, extract-validation, YouTube fallback/transcriber helpers, process/fallback flow, HTTP reader, Wayback, Sucuri protected fetch, WordPress recovery, and HTML extraction code, split syncjob types/options/progress/merge/X-frontier/stage-execution/runner-hook helpers, split ask query hints/evidence/scoring/prompt/entity/excerpt helpers, split brain research pack types/strategy/evidence ranking/topic/coverage/exact-tag/next-step helpers, split MCP get payload/section/related/format helpers, split entity indexing derivation/relationships/builder/path helpers, split topic map graph/entity/pivot/format/type helpers, split vault item rendering/media/quoted-post/YAML/text/option helpers, split GitHub importer transport/item/source/extract helpers, split YouTube importer feed/process cleanup/item/enrichment/browser helpers, split Safari tabs run/query/item/device/progress helpers, split X photo OCR orchestration/persistence/provider/options/helpers, split summarize CLI direct-provider/command/version/env helpers, split item categorization content/photo/LLM/config/tag helpers, and moved sync flag binding, env resolution, option assembly, summary rendering, stats output rendering, sync UI formatting, Apple Notes import output/debug subcommands, serve MCP/remote/web wiring, and source categorization command wiring out of larger app command bodies without changing route, tool, store, sync, retrieval, X hydration, or enrichment behavior.
...
- **Location**: `internal/store/`, `internal/sourceenrich/`, `internal/app/sync.go`, `internal/syncjob/`, `internal/xapi/`, `internal/ask/`, `internal/brainresearch/`, `internal/entities/`, `internal/topics/`, `internal/vault/`, `internal/githubimport/`, `internal/youtubeimport/`, `internal/safaritabs/`, `internal/xphotoocr/`, `internal/summarizecli/`, `internal/itemcategorize/`, `internal/mcpserver/`, `web/`, `README.md`, `docs/`
