...
- **Web/MCP/store/source structure**: Split the web server into focused route files, separated MCP transport/protocol/tools/schemas/helpers, moved store open/schema/search/predicate/item-read/item-scan/item-write/X-hydration/X-media-transcription and source schema/link/enrichment/read/search/repair code into focused files, extracted source-enrichment options, workers, persistence, summary execution/prompt/policy, failure persistence/classification/preflight, extract-validation, YouTube fallback, process/fallback flow, HTTP reader, Wayback, Sucuri protected fetch, WordPress recovery, and HTML extraction code, split syncjob types/options/progress/merge/X-frontier/stage-execution/runner-hook helpers, and moved sync flag binding, env resolution, option assembly, and summary rendering out of the CLI command body without changing route, tool, store, sync, or enrichment behavior.
