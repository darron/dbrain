...
- **Web/MCP/store/source structure**: Split the web server into focused route files, separated MCP transport/protocol/tools/schemas/helpers, moved store open/schema/search/predicate/item-read/item-scan/item-write/X-hydration/X-media-transcription/source-read/source-search/source-repair code into focused files, and extracted source-enrichment progress tracking without changing route, tool, store, or enrichment behavior.
