...
`categorize.model`. Create that wrapper on each machine with
`ollama pull qwen3.6:35b-a3b-nvfp4` followed by
`ollama create dbrain:2026042701 -f Modelfile`. `--local-model-profile
dbrain-omlx` writes the tested oMLX equivalent
`omlx/Qwen3.6-35B-A3B-MLX-4bit`, and `--local-model-profile small-ollama`
writes `ollama/gemma4:12b-mlx` for lower-memory installs. The installer does
not pull, create, or start model weights; prepare the model in the selected
runner first.
