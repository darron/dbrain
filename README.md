...
and `categorize.model`. Build that wrapper on each machine from the repo
`Modelfile`. The tested oMLX equivalent uses Qwen3.6 35B A3B MLX 4-bit, and
the smaller Ollama profile uses Gemma 4 12B MLX for machines where memory is
the constraint:

```sh
ollama pull qwen3.6:35b-a3b-nvfp4
ollama create dbrain:2026042701 -f Modelfile
...
The installer writes config only; it does not pull, push, or create model
weights. To bypass the profile or use a custom tag, pass the provider-qualified
model directly:
...
The supported dbrain wrapper setup is local creation from the repo `Modelfile`.
This avoids requiring a redistributed Modelfile-derived registry artifact:
...
local `ollama create ... -f Modelfile` path above, use the comparable
