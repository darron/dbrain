...
|-----------|---------------------|------------|---------------------|
| `dbrain audit all --profile deep` | None. Downloads and validates an archive candidate only inside a private temporary directory; it never calls the restore path or replaces `brain.db`, its WAL, or SHM. It also inventories configured upstream import sources without importing, deleting, repairing, retrying, or changing upstream state. | Explicit deep profile; compressed, decompressed, temporary-space, object, page, request, idle-read, per-source, identity, and whole-run limits; temporary files are cleaned on every exit. | Not part of `sync all`; deep is unavailable to scheduled audit, MCP, and admin runners. |
| `dbrain audit apple-notes\|safari-tabs\|x-bookmarks\|github-stars\|youtube-liked\|youtube-watch-later\|feeds` | None. Runs only the named source's bounded parity inventory against the query-only local snapshot. Apple Notes and Safari copy DB/WAL/SHM inputs to interruptible dbrain-owned snapshots; remote adapters do not write importer state. | Effective deep profile; one five-minute source context; 100,000 unique-identity and 10,000-page ceilings; content-free report; ambiguous or incomplete traversal is `unknown`. | Not part of `sync all`; scheduled source disablement does not prevent an explicit audit, and remains visible as `poll=skipped(feature_disabled)`. |
| `serve remote` scheduled audit | Writes immutable content-free reports and content-free transition state below private `<log_dir>/audit/`; optional alerts send a bounded fixed JSON body. It never changes SQLite, vault, media, OKF, or remote archives. | Disabled by default; fast/standard only; one in-flight audit; exact-profile alert debounce; generated no-follow paths; 90-day/256-MiB report retention; exact-origin no-proxy/no-redirect webhook policy. | Separate sibling scheduler. Fast runs only after an actual scheduled sync result and lock settlement; its outcome cannot replace the sync result. |
...
| `dbrain repair pruned-media` | Dry-run changes nothing. Only explicit `--apply` performs network requests and database/media writes while re-downloading archived, locally pruned media needed by pending OCR or transcription work. It does not run enrichment itself. | Dry-run is the default and opens SQLite read-only without migrations. `--apply` is required for writes; category limits are independently capped at 5,000 and per-media timeout defaults to 45 seconds. Existing workers enrich restored media afterward. | Not part of `sync all`; no automatic repair behavior is added. |
