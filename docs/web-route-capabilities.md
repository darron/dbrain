...
| `/api/links` | `POST` | Write DB; optional remote fetch/model call | Adds one or more URLs. When `defer: true`, validated captures are durably queued and the route returns `202` before feed discovery or enrichment; otherwise existing synchronous behavior applies. The deferred worker later discovers/imports feeds or creates an ordinary source; the next `sync all` sources stage extracts and summarizes that source. If `scheduler.sync_all.enabled` is set, the run follows `scheduler.sync_all.interval` (default `1h`); `dbrain web` alone does not run a scheduler. `enrich` has no immediate stats payload on a deferred `202`. |
