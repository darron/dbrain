...
func TestValidateReportAcceptsFrozenV1ReportAfterV2BecomesCurrent(t *testing.T) {
	now := time.Date(2026, 7, 14, 1, 2, 3, 0, time.UTC)
	report := NewReport(ProfileStandard, now)
	report.Schema = "dbrain.audit.v1"
	report.AuditID = "audit_20260714T010203.000000000Z_00000001"
	...
	report.Boundary.Layout = "xdg"
	report.CompletedAt = now.Add(time.Second)
	report.Checks = []Check{{ID: CheckBoundaryConfig, Category: CategoryBoundary, Status: StatusPass, Confidence: ConfidenceHigh, Required: true, Summary: fixedSummary(CheckBoundaryConfig), ObservedAt: now, Evidence: Evidence{"layout": "xdg", "config_source": "default", "verified": true}}}
	FinalizeReport(&report)
	if err := ValidateReport(report); err != nil {
		t.Fatalf("frozen v1 report rejected: %v", err)
	}
}
