...
- ` + "`dbrain_research_pack`" + `: bundle trace-free retrieve-only evidence, query/tag hints, exact tag and corpus coverage counts, optional semantic retrieval diagnostics, suggested follow-ups, and an optional topic brief
...
1. Research: call ` + "`dbrain_research_pack`" + ` first, check ` + "`coverage.recall_note`" + `, exact tag counts, ` + "`query_plan.semantic_mode`" + `, and retrieval-lane status, then inspect the strongest hits with ` + "`dbrain_get`" + ` using ` + "`content_mode=evidence`" + ` or expand with ` + "`dbrain_related`" + `. The configured semantic mode defaults to ` + "`off`" + `; ` + "`use_semantic`" + ` forces on, ` + "`disable_semantic`" + ` forces off, and both together are an error. Shadow results remain lexical-identical and expose only bounded content-free comparison metadata; on-mode evidence may include RRF ` + "`fused_score`" + ` plus chunk/content-section and full lane provenance. The optional lane is local Ollama plus SQLite-authoritative exact scan, with a default cap of 25,000 current ready embeddings for the configured profile counted before request filters; failures and oversized profile sets fail open to lexical evidence. Different-language recall depends on the embedding model and supporting evals. ANN, other providers, background sync, and default-on are deferred. Direct MCP pack calls never write research traces. Exact-tag evidence is a separate representative lane. Answer from the collector's intentionally selective saved corpus; prioritize accuracy over appearing objective, and do not add outside balance unless asked.
