...
2. Browse: call ` + "`dbrain_get`" + ` on a known item or source, then expand with ` + "`dbrain_related`" + `. Prefer DB-backed modes (` + "`brief`" + `, ` + "`evidence`" + `, ` + "`raw`" + `); media enrichments appear as ` + "`x_media_transcript`" + ` and ` + "`ocr_text`" + ` sections. Source ` + "`backlinks`" + ` include the referencing saved item's ` + "`user_tags`" + `, which often explain why an otherwise untagged source was saved. Use ` + "`rendered`" + ` for note shape only.
