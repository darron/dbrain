...
func TestReadinessPriorityAndBoundaries(t *testing.T) {
	...
	tests := []struct {
		...
		{"runtime aggregate drift is corrupt", func(s *Snapshot) { s.AggregateCountersCorrupt = true }, StateCorrupt, false},
	...
	catchTests := []struct {
		...
		{"current chunks above exact cap", func(s *Snapshot) { s.ChunkCount = 25_001 }, StateNeedsProjection},
