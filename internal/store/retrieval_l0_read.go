...
func (s *Store) ReadRetrievalExactL0(ctx context.Context, input RetrievalActiveRootReadRequest, limit int) ([]RetrievalEmbeddingRow, error) {
	...
	result, err := readRetrievalExactL0(ctx, tx, input, limit)
	if err != nil {
		return nil, err
	}
...
func normalizeRetrievalActiveRootReadRequest(input RetrievalActiveRootReadRequest) RetrievalActiveRootReadRequest {
...
func readRetrievalExactL0(ctx context.Context, reader sqlQueryer, input RetrievalActiveRootReadRequest, limit int) ([]RetrievalEmbeddingRow, error) {
	var generation string
	var epoch, snapshot, globalEpoch int64
	if err := reader.QueryRowContext(ctx, `SELECT active_generation_id,purge_epoch,active_snapshot_revision FROM retrieval_embedding_profiles WHERE profile_id=?`, input.ProfileID).Scan(&generation, &epoch, &snapshot); err != nil {
	...
	if err := reader.QueryRowContext(ctx, `SELECT purge_epoch FROM retrieval_state WHERE singleton=1`).Scan(&globalEpoch); err != nil {
