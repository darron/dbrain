...
func TestDetailsFromBuildInfo(t *testing.T) {
	...
	if details.GitVersion != "2.54.0" {
		t.Fatalf("GitVersion = %q", details.GitVersion)
	}
...
func TestInfoIncludesRequiredFields(t *testing.T) {
	...
	info := Info()
	for _, field := range []string{"commit", "short", "build_time", "git_status", "go_version", "git_version", "build_platform"} {
		if _, ok := info[field]; !ok {
			t.Fatalf("Info() missing %q: %#v", field, info)
		}
	}
	if info["git_version"] != GitVersion {
		t.Fatalf("git_version = %v, want %q", info["git_version"], GitVersion)
	}
