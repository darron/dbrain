...
- When inspecting a `src:...` source, read its `backlinks` rows too. Backlinks carry the saved item's `user_tags`, which often explain why the collector saved that source even when the source document itself has no tags.
- Use `user_tags` as retrieval hints. Tags can match searches, disambiguate broad topics, and indicate the user's own categorization, but they do not replace source text. When `exact_tag_evidence` is present, treat it as representative saved-item examples for the matching tag lane, not as a complete list.
