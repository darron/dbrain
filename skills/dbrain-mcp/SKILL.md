...
- Inspect `query_plan.semantic_mode`, `query_plan.retrieval_lanes`, and shadow-only `query_plan.shadow_comparison`. The foundation uses SQLite-authoritative exact vector scan with candidate depth 50 and a default 25,000-active-chunk cap. `too_large` and validly configured provider/search failures fail open to lexical evidence with explicit status/reason. ANN, other providers, background sync, and default-on are not available. Do not assume semantic retrieval ran unless the lane status says `used`.
...
- MCP exposes no semantic mutation or purge tool. Existing explicit parent-forget flows outside MCP, such as Apple Notes `--forget-excluded`, synchronously delete that parent's derived retrieval chunks and embeddings and stale affected retrieval generations with the established authoritative-content purge.
